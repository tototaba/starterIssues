...
import { createWorkOrder, updateWorkOrderById } from '../../api/client';
...
import { BaseCreateEditPage, CreateEditPageProps } from '../config/base';
...
export const CreateEditWorkOrder = ({
  open,
  current,
  close,
}: CreateEditPageProps<WorkOrderRecord>) => {
  const initialValues = {
    warehouseCode: current?.warehouseCode || '',
    warehouseName: current?.districtId || '',
    accountId: current?.districtCode || '',
    fercAccount: current?.warehouseId || '',
  ...
  const createEditMutation = useFleetMutation(
    (warehouseData: typeof initialValues) =>
      current
        ? updateWorkOrderById(current.id, {
          ...warehouseData,
          id: current.id,
          version: current.version,
        })
        : createWorkOrder(warehouseData)
  );
...
export default CreateEditWorkOrder;
